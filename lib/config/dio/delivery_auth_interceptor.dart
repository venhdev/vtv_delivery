...
class DeliveryAuthInterceptor extends QueuedInterceptor {
  final _innerDio = Dio();

  @override
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    //> add accessToken if needed
    if (options.path.contains('/shipping')) {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    log('got error when ${err.requestOptions.method} ${err.requestOptions.path}  : statusCode:${err.response?.statusCode} - data:${err.response?.data} - type:${err.type}');
